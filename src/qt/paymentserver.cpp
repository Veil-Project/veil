...
const char* BIP71_MIMETYPE_PAYMENT = "application/veil-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/veil-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/veil-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the veil: URI contains a payment request, we are not able to detect the
...
//
// OSX-specific way of handling veil: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in veil: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom veil addresses are not supported
        ...
        // Veil amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
