    ...
    const uint8_t **pcm_in, const uint8_t **pcm_out, const uint8_t **blinds)
{
    ...
    */
        ...
        for (k = 0; k < nCols; ++k) {
            ...
            if (secp256k1_gej_is_infinity(
                    &accj)) { /* With no blinds set, sum input commitments == sum output commitments */
    ...
    size_t nRows, size_t index, const uint8_t **sk, const uint8_t *pk)
{
    ...
    */
        ...
        for (k = 0; k < dsRows; ++k) {
            ...
            secp256k1_gej_set_ge(&gej1, &ge1);
            secp256k1_ecmult(&ctx->ecmult_ctx, &gej2, &gej1, &alpha[k],
    ...
    const uint8_t *pk, const uint8_t *ki, const uint8_t *pc, const uint8_t *ps)
{
        ...
        return secp256k1_scalar_is_zero(&zero) ? 0 : 8; /* return 0 on success, 2 on failure */
}
