...
// OpenSSL initialization removed (upstream 0.19/0.20): the RNG is now
// self-contained in random.cpp (RNGState) and nothing else in the core
// codebase uses OpenSSL.
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "veild -foo=bar
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Veil";
#else
    fs::path pathRet;
...
std::string ArgsManager::GetChainName() const
{
    ...
    if ((fTestNet && fRegTest) || (fDevNet && fRegTest) ||  (fTestNet && fDevNet))
        throw std::runtime_error("Invalid combination of -regtest, -testnet, and -devnet.");
    if (fRegTest)
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Veil Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Veil Core Developers";
    }
