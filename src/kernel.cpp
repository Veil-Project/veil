    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if(fModifierV2)
            hashProof = UintToArith256(pindex->GetBlockHash());
        else
            hashProof = pindex->IsProofOfStake() ? 0 : UintToArith256(pindex->GetBlockHash());

        CDataStream ss(SER_GETHASH, 0);
        ss << ArithToUint256(hashProof) << nStakeModifierPrev;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    if (gArgs.GetBoolArg("-printstakemodifier", false))
        LogPrintf("ComputeNextStakeModifier: prev modifier= %s time=%s\n", boost::lexical_cast<std::string>(nStakeModifier).c_str(), FormatISO8601DateTime(nModifierTime).c_str());
    ...
    for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
        ...
        if (gArgs.GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                nRound, FormatISO8601DateTime(nSelectionIntervalStop).c_str(), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
    ...
    if (gArgs.GetBoolArg("-printstakemodifier", false)) {
        LogPrintf("ComputeNextStakeModifier: new modifier=%s time=%s\n", boost::lexical_cast<std::string>(nStakeModifierNew).c_str(), FormatISO8601DateTime(pindexPrev->GetBlockTime()).c_str());
    }
...
{
    const CTransaction tx = *block.vtx[1].get();
    if (!tx.IsCoinStake())
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        CBlockIndex pBlockIndex(block);
        if (!GetTransaction(txin.prevout.hash, txPrev, Params().GetConsensus(), hashBlock, true, &pBlockIndex))
            return error("CheckProofOfStake() : INFO: read txPrev failed");

        //verify signature and script
        ScriptError err;
        CAmount amount = tx.GetValueOut();
        std::vector<uint8_t> vchAmount(8);
        memcpy(&vchAmount[0], &amount, 8);
        if (!VerifyScript(txin.scriptSig, txPrev->vout[txin.prevout.n].scriptPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&tx, 0, vchAmount), &err))
            return error("CheckProofOfStake() : VerifySignature failed on coinstake %s", tx.GetHash().ToString().c_str());

        CPivStake* pivInput = new CPivStake();
        pivInput->SetInput(*(txPrev.get()), txin.prevout.n);
