...
void PrivateCoin::mintCoin(const CoinDenomination denomination) {
	...
	for(uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
        ...
        bool isValid = false;
        while (!isValid) {
            isValid = GenerateKeyPair(this->params->coinCommitmentGroup.groupOrder, ArithToUint256(0), key, s);
        }
...
int ExtractVersionFromSerial(const CBigNum& bnSerial)
{
	...
	arith_uint256 nMark = bnSerial.getarith_uint256() >> (256 - PrivateCoin::V2_BITSHIFT);
	if (nMark == 0xf)
